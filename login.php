...
error_reporting(0);
 ...
 session_start();
if($_SESSION['admin']){
 header("Location: admin/espacioadmin.php");
}
else if($_SESSION['profesor']){
   header("Location: profesor/espacioprofesor.php");
}
else if($_SESSION['director']){
   header("Location: director/espaciodirector.php");
} 
 ?>
<!DOCTYPE html>
      ...
      <form name="form_login" method="post" action="manejadorsesiones.php" role="form">
        <fieldset>
          ...
          <div class="form-group">
            <input name="usuario" type="text" id="usuario" class="form-control input-lg" placeholder="Email Address">
          </div>
          <div class="form-group">
